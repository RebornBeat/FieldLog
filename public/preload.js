...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // Events from main
  onMenuAction:         (cb)  => { const h=(_,d)=>cb(d); ipcRenderer.on('menu:action',h); return ()=>ipcRenderer.off('menu:action',h); },
  onWindowStateChange:  (cb)  => { const h=(_,d)=>cb(d); ipcRenderer.on('window:state-change',h); return ()=>ipcRenderer.off('window:state-change',h); },
});
